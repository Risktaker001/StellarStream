...
import { usePendingStreams, type PendingStream } from "@/lib/use-pending-streams";
      ...
      {signers.map((signer, i) => {
        ...
        return (
          <div
            ...
            className={`relative flex h-6 w-6 items-center justify-center rounded-full border text-[10px] transition-all duration-500 ${
              isSigned
...
// ─── Desktop Card (non-swipe fallback) ────────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        {!stream.hasCurrentUserSigned && !isFullySigned && (
          <button
            onClick={onSign}
            disabled={isSigning}
            className={`flex-shrink-0 flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all duration-200 ${
              isSigning
      ...
      <div className="flex items-center justify-between text-[11px] text-white/30">
        <span>Created {timeUntil(stream.createdAt)}</span>
        <span
...
export default function SwipeApprovalQueue() {
  const { streams, signingIds, lastRefreshed, signStream, signedCount, restartProposal } =
    usePendingStreams();
  ...
  const handleRestart = async (stream: PendingStream) => {
    ...
    try {
      ...
      toast.success({
        title: "Proposal Restarted",
        description: `All signatures for ${stream.streamId} have been cleared.`,
        duration: 5000,
      });
    } catch {
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-purple-500/30 bg-purple-500/10 px-2.5 py-0.5 text-[10px] font-bold tracking-widest text-purple-400 uppercase">
                  <Smartphone size={10} />
                  Swipe Enabled
                </span>
              </div>
            ...
            <p className="font-body mt-1 text-sm text-white/40 max-w-lg">
              Swipe right to approve, left to reject. High-value transactions require biometric
              confirmation.
            </p>
          </div>
        ...
        <div className="mt-5 flex gap-1 rounded-xl border border-white/10 bg-white/[0.03] p-1 w-fit">
          {(
            [
              ...
              {
                key: "conflict",
                label: `Conflicts${conflictCount > 0 ? ` (${conflictCount})` : ""}`,
              },
            ] as const
          ).map((tab) => (
            <button
              ...
              className={`rounded-lg px-4 py-1.5 text-sm font-body transition-all duration-200 ${
                filter === tab.key
